...
if (isset($_POST['update_cats'])) {
    ...
    $update_cats = $conn->prepare("UPDATE `tbl_category` SET title = ? WHERE id = ?");
    $update_cats->execute([$title,  $c_id]);
                ...
                <div class="boxes">
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            $select_cats = $conn->prepare("SELECT * FROM `tbl_category` WHERE id = ?");
                            $select_cats->execute([$update_id]);
                            if ($select_cats->rowCount() > 0) {
                                while ($fetch_cats = $select_cats->fetch(PDO::FETCH_ASSOC)) {

                            ?>
                                    <form action="" method="post" enctype="multipart/form-data">
                                        <input type="hidden" name="c_id" value="<?= $fetch_cats['id']; ?>">
                                        <div class="form-group">
                                            <label for="title">التصنيف</label>
                                            <input type="text" class="form-control" id="title" name="title" value="<?= $fetch_cats['title']; ?>" placeholder="اسم المنتج">
                                        </div>

                                        <div class="form-group">
                                            <input type="submit" name="update_cats" class="btn btn-success" style="float:left">
                                        </div>
                                    </form>
